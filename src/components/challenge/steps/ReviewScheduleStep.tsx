...
import { ArrowLeft, Calendar, Clock, Sparkles, Info, Trophy, Users, Globe, Lock } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="glass-card p-6 space-y-4">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h3 className="text-lg font-medium text-foreground">Challenge Summary</h3>
        </div>

        <div className="space-y-3">
          <div>
            <h4 className="text-sm font-medium text-primary">{data.name}</h4>
            <p className="text-xs font-light text-muted-foreground mt-1">{data.description}</p>
          </div>
          ...
          <div className="glass-card p-3 bg-primary-soft/10">
            <div className="text-xs font-medium text-primary mb-1">
              {data.challengeType === 'single-task' ? '1 Goal' : `${data.stages.length} Stages`}
            </div>
            <div className="text-xs font-light text-muted-foreground">
              Total rewards: {data.stages.reduce((total, stage) => {
                return total + (stage.points || 0) + (stage.gems || 0);
              }, 0)} points/gems
            </div>
          </div>
        </div>
      ...
      <div className="glass-card p-6 space-y-6">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="text-lg font-medium text-foreground">Schedule Settings</h3>
        </div>

        {/* Publish Option */}
        <div className="space-y-3">
          <label className="text-sm font-medium text-primary">When to publish challenge?</label>
          ...
          {data.publishOption === 'scheduled' && (
            <div className="ml-7">
              <input
                ...
                value={data.publishDate ? data.publishDate.toISOString().slice(0, 16) : ''}
                onChange={(e) => onUpdate({ publishDate: new Date(e.target.value) })}
              />
              {errors.publishDate && (
                <p className="text-xs text-destructive mt-1">{errors.publishDate}</p>
              )}
            </div>
          )}
        </div>
        ...
        {/* Start Option */}
        <div className="space-y-3">
          <label className="text-sm font-medium text-primary">When does the challenge start?</label>
          <div className="space-y-2">
            <label className="flex items-center space-x-3 cursor-pointer">
              <input
                type="radio"
            ...
            <label className="flex items-center space-x-3 cursor-pointer">
              <input
                ...
                className="w-4 h-4 text-primary"
              />
              <span className="text-sm text-foreground">Start on specific date</span>
            </label>
          </div>
          ...
          {data.startOption === 'scheduled' && (
            <div className="ml-7">
              <input
                type="datetime-local"
                className="soft-input w-full"
                value={data.startDate ? data.startDate.toISOString().slice(0, 16) : ''}
        ...
        {/* End Date */}
        <div className="space-y-2">
          <label className="text-sm font-medium text-primary">When does the challenge end? *</label>
          <input
            type="datetime-local"
            className="soft-input w-full"
      ...
      <div className="glass-card p-4 bg-primary-soft/20">
        <div className="flex items-start space-x-3">
          ...
          <p className="text-xs font-light text-muted-foreground">
            <span className="font-medium text-primary">Ready to launch:</span> Once created, you can edit challenge details before the start date.
          </p>
        </div>
      </div>
    </div>
